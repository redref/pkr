...
class Kard(object):
    ...
    def create(cls, args):
        ...
        kard_path = cls._build_kard_path(args.name)
        kard_path.mkdir(exist_ok=True)

        try:
            features = extras.pop("features") if "features" in extras else []
            meta = {"env": args.env, "driver": {"name": args.driver}, "features": features}
            ...
            kard = cls(args.name, kard_path, meta)
    ...
    def pull_images(self, args):
        """Pull images"""
        services = args.services or list(self.env.get_container().keys())
        registry = self.docker_cli.get_registry(
            url=args.registry, username=args.username, password=args.password
        )
        self.docker_cli.pull_images(services, registry, tag=args.tag, parallel=args.parallel)
    ...
    def import_images(self, args):
        services = args.services or list(self.env.get_container().keys())
        self.docker_cli.import_images(services, tag=args.tag)

    def list_images(self, args):
        """List images"""
        services = args.services or list(self.env.get_container().keys())
        if args.tag is None:
            args.tag = self.meta["tag"]
        for service in services:
            write(self.docker_cli.make_image_name(service, args.tag))

    def purge_images(self, args):
        """Purge images"""
        self.docker_cli.purge(args.except_tag, args.tag, args.repository)
